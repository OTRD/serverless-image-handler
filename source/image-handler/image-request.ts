...
import {DefaultImageRequest, Headers, ImageEdits, ImageFormatTypes, ImageHandlerError, ImageHandlerEvent, ImageRequestInfo, RequestTypes, StatusCodes} from './lib';
  ...
  constructor(private readonly s3Client: S3, private readonly secretProvider: SecretProvider) {}
    ...
    public async getOriginalImage(bucket: string, key: string): Promise<OriginalImageInfo> {
        try {
            ...
            try {
                ...
                try {
                    await new Promise((resolve, reject) => {
                        client.get(key, res => {
                            ...
                            if (res.statusCode.toString().startsWith('30')) {
                                client.get(res.headers.location, response => {
                                    response.setEncoding('binary');
                                    response.on('data', chunk => imageInfo[0] += chunk);
                                    response.on('end', () => {
                                        imageInfo[1] = response.headers;
                                        resolve(imageInfo);
                                    });
                                }).on('error', reject);
                            } else {
                                res.on('data', chunk => imageInfo[0] += chunk);
                                res.on('end', () => {
                                    imageInfo[1] = res.headers;
                                    resolve(imageInfo);
                                });
                            }
                        }).on('error', reject);
                ...
                originalImage = {
                    'Body': Buffer.from(imageInfo[0], 'binary'),
                    'ContentType': imageInfo[1]['content-type'],
                    'LastModified': new Date(imageInfo[1]['last-modified']),
                };
            }
    ...
    public parseImageKey(event: ImageHandlerEvent, requestType: RequestTypes): string {
        ...
        if (requestType === RequestTypes.THUMBOR || requestType === RequestTypes.CUSTOM) {
            ...
            return decodeURIComponent(path.replace(/\/((?:adaptive-|full-)?fit-in\/)?(\d+x\d+\/)?(\d+x\d+:\d+x\d+\/)?((filters:[^\/.]+\/?)*)?(smart\/)?/, ''));
        }
    ...
    public parseRequestType(event: ImageHandlerEvent): RequestTypes {
        ...
        const matchThumbor = /^(\/?)((?:adaptive-|full-)?fit-in?|(filters:.+\(.?\))?|(unsafe)?)(((.(?!(\.[^.\\/]+$)))*$)|.*(\.jpg$|.\.png$|\.webp$|\.tiff$|\.tif$|\.jpeg$|\.svg$))/i;
        const {REWRITE_MATCH_PATTERN, REWRITE_SUBSTITUTION} = process.env;
